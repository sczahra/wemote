...
async function api(path, options={}) {
  ...
  if (!base) throw new Error("Enter the remote bridge URL above.");
...
async function refreshStatus() {
  try {
    ...
    $("bridgeStatus").textContent = "Bridge online";
    if ($("appVersion")) $("appVersion").textContent = s.version || "0.4.2";
    $("deviceName").textContent = s.device ? ` | ${s.device.name}` : " | No Wemo selected";
    $("schedulerState").textContent = s.scheduler?.running ? "RUNNING" : "STOPPED";
    $("schedulerState").className = s.scheduler?.running ? "status-good" : "status-bad";
    renderMode(s.mode);

    const state = s.device?.state;
    const stateText = state === 1 ? "ON" : state === 0 ? "OFF" : "Unknown";
    $("masterState").textContent = stateText;
    $("masterState").className = "master-state " + (state === 1 ? "is-on" : state === 0 ? "is-off" : "");
    ...
    $("bridgeStatus").textContent = currentBridgeUrl() ? "Bridge unavailable" : "Bridge URL required";
...
async function refreshEvents() {
  try {
    ...
    const box = $("events");
    box.innerHTML = "";
    if (!data.events.length) { box.textContent = "No activity yet."; return; }
    data.events.forEach(ev => {
      const row = document.createElement("div"); row.className = "event";
      const t = document.createElement("span"); t.textContent = new Date(ev.created_at).toLocaleString();
      const d = document.createElement("span"); d.textContent = `${ev.event_type}: ${ev.detail || ""}`;
      row.append(t,d); box.appendChild(row);
    });
  } catch {}
}
...
$("saveBridgeUrl").onclick = async () => {
  ...
  if (!/^https:\/\/[a-zA-Z0-9.-]+$/.test(raw)) {
    showMessage("Enter the full HTTPS tunnel URL.", true);
    return;
  }
...
$("feederMode").onclick = async () => { try { await api("/api/mode", {method:"POST", body:JSON.stringify({mode:"feeder"})}); await refreshStatus(); showMessage("Pet Feeder Mode enabled. Plug forced OFF for safety."); } catch(e){showMessage(e.message,true);} };
$("lightMode").onclick = async () => { try { await api("/api/mode", {method:"POST", body:JSON.stringify({mode:"light"})}); await refreshStatus(); showMessage("Light Timer Mode enabled."); } catch(e){showMessage(e.message,true);} };
$("masterOn").onclick = async () => { try { await api("/api/power", {method:"POST", body:JSON.stringify({state:"on"})}); showMessage("Master power ON."); await refreshStatus(); await refreshEvents(); } catch(e) { showMessage(e.message, true); } };
$("masterOff").onclick = async () => { try { await api("/api/power", {method:"POST", body:JSON.stringify({state:"off"})}); showMessage("Master power OFF."); await refreshStatus(); await refreshEvents(); } catch(e) { showMessage(e.message, true); } };
$("feedNow").onclick = async () => { const b=$("feedNow"); b.disabled=true; b.textContent="FEEDING..."; try { const r=await api("/api/feed", {method:"POST"}); showMessage(`Feed cycle complete (${r.pulse_seconds}s pulse).`); await refreshStatus(); await refreshEvents(); } catch(e){showMessage(e.message,true);} finally {b.disabled=false;b.textContent="FEED NOW";} };
$("saveSettings").onclick = async () => {
  ...
  try { await api("/api/settings", {method:"POST", body:JSON.stringify(body)}); showMessage("Settings saved locally on this bridge."); await refreshStatus(); await refreshEvents(); } catch(e){showMessage(e.message,true);}
};
$("refreshDevices").onclick = async () => { await refreshDevices(); await refreshStatus(); };
$("deviceSelect").onchange = async (e) => { if (!e.target.value) return; try { await api("/api/device", {method:"POST", body:JSON.stringify({device_id:e.target.value})}); showMessage("Wemo selected."); await refreshStatus(); } catch(err){showMessage(err.message,true);} };
