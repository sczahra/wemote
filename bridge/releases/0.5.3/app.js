...
function renderDays() {
  ...
  dayNames.forEach((name, i) => {
    ...
    b.textContent = name;
    b.onclick = () => { selectedDays.has(i) ? selectedDays.delete(i) : selectedDays.add(i); settingsDirty = true; renderDays(); };
    wrap.appendChild(b);
  });
}
...
async function refreshStatus() {
  try {
    ...
    if ($("appVersion")) $("appVersion").textContent = s.version || "0.5.3";
    $("deviceName").textContent = s.device ? ` | ${s.device.name}` : " | No Wemo selected";
    $("schedulerState").textContent = s.scheduler?.running ? (s.settings?.schedule_enabled ? "RUNNING | ENABLED" : "RUNNING | SCHEDULE OFF") : "STOPPED";
...
async function refreshDevices() {
  try {
    ...
    if (!data.devices.length) {
      const o = document.createElement("option"); o.textContent = "No Wemo devices found"; o.value = ""; select.appendChild(o);
      return;
    }
    data.devices.forEach(d => {
      const o = document.createElement("option");
      o.value = d.id; o.textContent = `${d.name} (${d.model})`;
...
$("saveSettings").onclick = async () => {
  ...
  try {
    ...
    settingsDirty = false;
    showMessage("Settings saved. Schedule reloaded from bridge.");
    await refreshStatus();
    await refreshEvents();
  } catch(e){showMessage(e.message,true);}
};
...
renderDays();
refreshStatus(); refreshDevices(); refreshEvents(); tickClock();
